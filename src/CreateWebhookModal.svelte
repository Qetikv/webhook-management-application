<script lang="ts">
  ...
  const arrowIcon = '⌵'; // Using a curved down arrow character
  ...
  let customHeaders: Array<{ key: string; value: string }> = [{ key: '', value: '' }];
  let basicAuth = {
    username: '',
    password: ''
  };
...
<style>
  ...
  /* Override width for basic auth inputs */
